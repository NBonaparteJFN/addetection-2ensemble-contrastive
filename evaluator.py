...
def getIndividualPredictions(task, model, feature, y, config, fold):
    if task == 'classification':
        ...
        report = precision_recall_fscore_support(
            np.argmax(y, axis=-1), preds, average='binary')
        print('precision: {:.3f}, recall: {:.3f}, f1_score: {:.3f}, accuracy: {:.3f}'.format(
            report[0], report[1], report[2], accuracy))
        ...
        y = np.array(y)
        score = mean_squared_error(np.expand_dims(
            y, axis=-1), preds, squared=False)
        print('fold ', fold)
        print('rmse ', score)
        return (score, preds)


def getIndividualEnsembleAccuracy(data, val_preds, config, num_classes=2, learnt_voter=None, fold=None, plot=False):
    ...
    if task == 'classification':
        probs = np.stack(val_preds, axis=1)

        if config.voting_type == 'hard_voting':
            model_predictions = np.squeeze(probs)
            pdb.set_trace()
            ...
            if learnt_voter is None:
                learnt_voter = LogisticRegression(C=0.1).fit(
                    model_predictions, np.argmax(y, axis=-1))
        ...
        report = precision_recall_fscore_support(
            np.argmax(y, axis=-1), voted_predictions, average='binary')
        print('precision: {:.3f}, recall: {:.3f}, f1_score: {:.3f}, accuracy: {:.3f}'.format(
            report[0], report[1], report[2], accuracy))
        ...
        if config.voting_type == 'hard_voting':
            ...
            if config.uncertainty:
                predictions = np.stack(val_preds, axis=1)		# N, #_of_fold, 1
                probs = np.mean(predictions, axis=1)			# 1, #_of_fold, 1
                pred_stds = np.std(predictions, axis=1)			# 1, #_of_fold, 1
                pred_entropies = scipy.stats.entropy(
                    predictions, axis=0)				# 1, #_of_fold, 1
                voted_predictions = np.mean(probs, axis=1)		# 1, 1, 1
            else:
                predictions = np.stack(val_preds, axis=1)
                voted_predictions = np.mean(predictions, axis=1)  # N, 1, 1
            ...
            # pdb.set_trace()
            pred_entropies = scipy.stats.entropy(
                predictions, axis=1)  # 1, #_of_fold, 1
            # pdb.set_trace()
            ...
            voted_predictions = np.sum(probs * voting_weights, axis=1)
            # pdb.set_trace()

        if config.uncertainty:
            average_uncertainties = np.squeeze(np.mean(pred_stds, axis=1))
            average_entropies = np.squeeze(np.mean(pred_entropies, axis=1))
        ...
        # pdb.set_trace()
        score = mean_squared_error(np.expand_dims(
            y, axis=-1), voted_predictions, squared=False)
...
def getPredictions(m, data, config, fold):
    print('Loading data...')
    ...
    if config.uncertainty:
        ...
        model_files = sorted(
            glob.glob(os.path.join(model_dir, '{}/*.h5'.format(m))))
        # pdb.set_trace()
        saved_models = list(map(lambda x: tf.keras.models.load_model(
            x, custom_objects={'negloglik': negloglik}), model_files))
    ...
    if dataset_split == 'full_dataset':  # compare features need to be projected
        ...
        # pdb.set_trace()
        if(m == "compare"):
            compare_train, compare_val = data['compare'][:
                                                         n_train], data['compare'][n_train:]
            sc = load(open(os.path.join(config.model_dir,
                      'compare/scaler_{}.pkl'.format(fold+1)), 'rb'))
            pca = load(open(os.path.join(config.model_dir,
                       'compare/pca_{}.pkl'.format(fold+1)), 'rb'))
        ...
        if m == 'compare':
            print('Train')
            train_accuracy, train_preds = getIndividualPredictions(
                task, saved_model_types[m][fold], compare_train, y_train, config, fold=fold)
            print('Val')
            val_accuracy, val_preds = getIndividualPredictions(
                task, saved_model_types[m][fold], compare_val, y_val, config, fold=fold)
        else:
            print('Train')
            train_accuracy, train_preds = getIndividualPredictions(
                task, saved_model_types[m][fold], data[m][:n_train], y_train, config, fold=fold)
            print('Val')
            val_accuracy, val_preds = getIndividualPredictions(
                task, saved_model_types[m][fold], data[m][n_train:], y_val, config, fold=fold)
...
def evaluate(data, test_data, config):
        ...
        for i in range(config.n_folds):
            ...
            if len(model_types) == 1:
                ...
                if m == 'compare':
                    ...
                    print('Train')
                    train_accuracy = get_individual_accuracy(
                        task, saved_model_types[m][fold], compare_train, y_train, config, fold=fold)
                    print('Val')
                    val_accuracy = get_individual_accuracy(
                        task, saved_model_types[m][fold], compare_val, y_val, config, fold=fold)
                    print('Test')
                    test_accuracy = get_individual_accuracy(
                        task, saved_model_types[m][fold], compare_test, y_test, config, fold=fold)
                else:
                    print('Fold {}'.format(fold+1))
                    print('Train')
                    train_accuracy = get_individual_accuracy(
                    ...
                    print('Test')
                    test_accuracy = get_individual_accuracy(
                        task, saved_model_types[m][fold], test_data[m], y_test, config, fold=fold)
            else:
                models = []
                features = []
                for m in model_types:
                ...
                train_accuracy, learnt_voter, _ = get_ensemble_accuracy(
                    task, models, features, y_train, config)
                ...
                # pdb.set_trace()
                val_accuracy, _, _ = get_ensemble_accuracy(
                    task, models, features, y_val, config, learnt_voter=learnt_voter,  fold=fold)
        ...
        for train_index, val_index in KFold(n_folds).split(y):
            ...
            if len(model_types) == 1:
                ...
                if m == 'compare':
                    ...
                    print('Train')
                    train_accuracy = get_individual_accuracy(
                        task, saved_model_types[m][fold], compare_train, y_train, fold=fold)
                    print('Val')
                    val_accuracy = get_individual_accuracy(
                        task, saved_model_types[m][fold], compare_val, y_val, fold=fold)

                else:
                    print('Fold {}'.format(fold+1))
                    print('Train')
                    train_accuracy = get_individual_accuracy(
                        task, saved_model_types[m][fold], data[m][train_index], y_train, fold=fold)
                    print('Val')
                    val_accuracy = get_individual_accuracy(
                        task, saved_model_types[m][fold], data[m][val_index], y_val, fold=fold)
                ...
                # pdb.set_trace()
                train_accuracy, learnt_voter = get_ensemble_accuracy(
                    task, models, features, y_train, config)
                ...
                # pdb.set_trace()
                val_accuracy, _ = get_ensemble_accuracy(
                    task, models, features, y_val, config, learnt_voter=learnt_voter,  fold=fold)
    ...
    if config.uncertainty and len(config.model_types) > 1:
        print('Test Average Uncertainties: ', list(
            np.mean(average_uncertainties, axis=0)))
        print('Test Average Entropies: ', list(
            np.mean(average_entropies, axis=0)))
...
def get_ensemble_accuracy(task, models, features, y, config, num_classes=2, learnt_voter=None, fold=None, plot=False):
    ...
    if task == 'classification':
        ...
        if config.voting_type == 'hard_voting':
            ...
            voted_predictions = [max(set(i), key=list(i).count)
...
def plot_entropy(entropies, fold, config):
    ...
    plt.savefig(os.path.join(config.model_dir,
                'plots/fold_{}.png'.format(fold)), dpi=300)
    plt.clf()
    plt.close()
