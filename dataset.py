...
import numpy as np
np.random.seed(0)
p = np.random.permutation(106) # n_samples = 108
...
def get_pause_features(transcription_filename, audio_filename, audio_length_normalization=10):
    ...
    with open(transcription_filename, 'r', encoding = "utf8") as f:
        content = f.read()
...
def get_intervention_features(transcription_filename, max_length=40):
    ...
    with open(transcription_filename, encoding="utf-8", errors="ignore") as f:
        content = f.read()
...
def prepare_data(dataset_dir, config):
	...
	#	pdb.set_trace()
	#	subjects = subjects[random_idx[0:subjects.shape[0]/2], :]
	...
	for filename in cc_files:
		...
		#print(get_intervention_features(filename, config.longest_speaker_length), "\n")
		#print(filename, "\n")
	...
	if config.isContrastive:
		random_idx = np.arange(X_intervention.shape[0])
		np.random.shuffle(random_idx)
		X_intervention = X_intervention[random_idx[0:(int)(X_intervention.shape[0]/2)], :]
		X_intervention = np.concatenate((X_intervention, AugmentDataWithGaussian(X_intervention)), axis=0).astype(np.float32)
	y_intervention = np.concatenate((y_cc, y_cd), axis=0).astype(np.float32)
	if config.isContrastive:
		y_intervention = y_intervention[random_idx[0:(int)(y_intervention.shape[0]/2)], :]
		y_intervention = np.concatenate((y_intervention, y_intervention), axis=0).astype(np.float32)
	elif config.isAugment_intervention:
		X_intervention = AugmentDataWithGaussian(X_intervention)
	...
	if config.isContrastive:
		y_reg_intervention = y_reg_intervention[random_idx[0:(int)(y_reg_intervention.shape[0]/2)]]
		y_reg_intervention = np.concatenate((y_reg_intervention, y_reg_intervention), axis=0).astype(np.float32)
	...
	if config.isContrastive:
		X_pause = X_pause[random_idx[0:(int)(X_pause.shape[0]/2)], :]
		X_pause = np.concatenate((X_pause, AugmentDataWithGaussian(X_pause)), axis=0).astype(np.float32)
	elif config.isAugment_disfluency:
		X_pause=AugmentDataWithGaussian(X_pause)
	y_reg_cc = utils.get_regression_values(os.path.join(dataset_dir, 'cc_meta_data.txt'))
	y_reg_cd = utils.get_regression_values(os.path.join(dataset_dir, 'cd_meta_data.txt'))
	...
	if config.isContrastive:
		y_reg_pause = y_reg_pause[random_idx[0:(int)(y_reg_pause.shape[0]/2)]]
		y_reg_pause = np.concatenate((y_reg_pause, y_reg_pause), axis=0).astype(np.float32)
	#pdb.set_trace()
	...
	y_pause = np.concatenate((y_cc, y_cd), axis=0).astype(np.float32).astype(np.float32)
	if config.isContrastive:
		y_pause = y_pause[random_idx[0:(int)(y_pause.shape[0]/2)], :]
		y_pause = np.concatenate((y_pause, y_pause),axis=0).astype(np.float32)
	...
	if config.isContrastive:
		#pdb.set_trace()
		X_compare = X_compare[random_idx[0:(int)(X_compare.shape[0]/2)], :]
		#pdb.set_trace()
		X_compare = np.concatenate((X_compare, AugmentDataWithGaussian(X_compare)),axis=0).astype(np.float32)
		#pdb.set_trace()
	elif config.isAugment_acoustic:
		X_compare = AugmentDataWithGaussian(X_compare)
...
def prepare_data_new(dataset_dir, config):
	...
	X_pause = np.concatenate((all_counts_cc, all_counts_cd), axis=0).astype(np.float32)
	if config.isContrastive:
		random_idx = np.random.shuffle(np.arange(X_pause.shape[0]))
		X_pause = X_pause[random_idx[0:(int)(X_pause.shape[0]/2)], :]
		X_pause = np.concatenate((X_pause, AugmentDataWithGaussian(X_pause)),axis=0).astype(np.float32)
	elif config.isAugment_disfluency:
		X_pause = AugmentDataWithGaussian(X_pause)
	...
	if config.isContrastive:
		y_reg_pause = y_reg_pause[random_idx[0:(int)(y_reg_pause.shape[0]/2)], :]
		y_reg_pause = np.concatenate((y_reg_pause, y_reg_pause),axis=0)
...
def prepare_test_data(dataset_dir, config):
	...
	if config.isAugment_intervention_Test:
		X_intervention = AugmentDataWithGaussian(X_intervention)
	...
	if config.isAugment_disfluency_Test:
		X_pause = AugmentDataWithGaussian(X_pause)
	...
	if config.isAugment_acoustic_Test:
		X_compare = AugmentDataWithGaussian(X_compare)
...
def AugmentDataWithGaussian(x):
	xShape = np.shape(x)
	mu = 0
	sigma = 0.1
	s = np.random.normal(mu, sigma, size=xShape)
	return s+x